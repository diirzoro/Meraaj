...
FINANCIAL_EVENTS = {e.event: e for e in (
    ...
    _E("dispute_resolution", "تسوية نزاع", "booking",
       ...
       (("seller_payable", "office_wallet_liability", "commission_revenue")),
       note="مغطى تفصيلاً بـ dispute_refund / dispute_release"),
    _E("dispute_opened", "فتح نزاع", "booking",
       "buyer opens a dispute on a dispatched booking", False, (),
       note="لا قيد: فتح النزاع لا يحرّك مالاً — الأثر المحاسبي عند الحسم فقط"),
    _E("dispute_refund", "حسم نزاع لصالح المشتري", "booking",
       "admin resolves the dispute by refunding the buyer", True,
       (("seller_payable", "deferred_platform_revenue", "office_wallet_liability")),
       implemented=True,
       note="إفراج عن مستحق البائع والإيراد المؤجّل وإعادة المبلغ للمشتري — لا إيراد"),
    _E("dispute_release", "حسم نزاع لصالح البائع", "booking",
       "admin resolves the dispute in the seller's favour (earning point)", True,
       (("seller_payable", "office_wallet_liability", "commission_revenue",
         "deferred_platform_revenue")), implemented=True,
       note="نفس لحظة الاستحقاق كالتسوية: الإيراد المؤجّل يصبح إيراداً وعمولة المسوّق "
            "تُحرَّر كالتزام له"),
    _E("commission_adjustment", "تعديل يدوي لعمولة المنصة", "booking",
       "admin overrides the platform fee on a live (unsettled) booking", True,
       (("office_wallet_liability", "deferred_platform_revenue")), implemented=True,
       note="الفرق يتحرك بين التزام محفظة المشتري والإيراد المؤجّل — وليس إيراداً"),
    _E("ads_capture", "تحصيل إعلان", "ad",
