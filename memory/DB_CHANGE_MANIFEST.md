...
## تحديث (جولة إصلاحات الإعلانات/الدفتر — بيئة التطوير فقط)
### حقول جديدة على مجموعة `advertisements` (إضافية، اختيارية، لا ترحيل ولا كتابة رجعية)
- `advertiser_owner_id` (str|null) — حساب المعلن، يُستبعد من جمهور الإعلان.
- `advertiser_org_id` (str|null) — مؤسسة المعلن، تُستبعد هي وكل مستخدميها.
- `audience_user_ids` (list[str]) و`audience_org_ids` (list[str]) — للاستهداف المحدد.
- `placement_group` (str|null) — اختصار يُوسَّع إلى قائمة صفحات عند الحفظ.
- `placements` الآن تقبل 12 صفحة من قائمة config قابلة للتوسعة (بدل 4).
- السجلات القائمة تعمل كما هي: الحقول الغائبة تُقرأ كـ null/[] ولا يظهر أي إعلان بلا مالك محدد
  لأي مستخدم إضافي بالخطأ (الاستبعاد يعتمد على تطابق صريح فقط).
### لا تغيير آخر
- لا مجموعات جديدة، لا فهارس جديدة، لا ترحيلات جديدة عند بدء التشغيل، ولا أي كتابة على
  `transactions` أو `bookings` أو المحافظ (كل إصلاحات الدفتر قراءة فقط).
