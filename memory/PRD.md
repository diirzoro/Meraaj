...
## Inbound price-warnings + batch re-sync — DONE Aug 2026
...
## P2P Approval/Escrow lifecycle — Phase 1 (backend) — DONE Aug 2026
Applies ONLY to `source:"rahal"` bookings (B2B + B2C). Manual/legacy bookings untouched (no `approval_status` → treated as approved; old cancel flow unchanged). No Migration.
- **New booking fields (rahal only):** `approval_status` (pending/approved/rejected/expired), `cancellation_status` (none/requested/withdrawn/expired), `approval_expires_at` (env `APPROVAL_TIMEOUT_HOURS`, default 24), `delivery_status` (pending/delivered/failed, reconciled from outbox on read — separate from business status).
- **create_booking (`market.py`):** rahal → HOLD buyer funds (debit available) + reserve seats + `approval_status=pending`; DEFER seller escrow / platform revenue / marketer commission until approval; sends `meraaj.booking.created` v2 (unchanged).
- **Inbound (`integration.py::rahal_webhook`):** `booking.approved` → establish deferred financials (seller pending escrow + revenue + commission; NO release to seller available — settlement still does that at green+24h). `booking.rejected` → full refund (reverses exact `debit_split` per-currency) + release seats + free passports + status cancelled. Match by `data.booking_ref = bookings._id`; reason from `data.reason` (fallback `rejection_reason` read-only). Idempotent via `event_id` + atomic state-claim (`find_one_and_update` on approval_status==pending).
- **Cancellation:** pending → buyer withdraw = full unwind + `meraaj.booking.cancelled`. approved → `cancellation_status=requested` + `meraaj.booking.cancellation_requested` (v2), NO refund/seats (final decision = Super Admin, Phase 2, PAUSED pending unified contract). Legacy blue/yellow flow unchanged + now frees `trip_passports`.
- **Passport dedup (§5):** unique index `trip_passports(package_id, passport_norm)`; normalized (upper, strip spaces/dashes); atomic backend enforcement; same passport different package allowed; freed on reject/withdraw/expire/legacy-cancel.
- **Lazy expiry (§9):** `_maybe_expire_pending` on read (no cron) → unwind + notify, idempotent.
- **Audit Trail:** append-only `booking_events` (+ `GET /api/bookings/{id}/timeline`). issue_visas blocked while approval_status pending/rejected/expired.
- **E2E PASS (all):** A create→pending, A→approved (escrow only after approve, no seller release), B→rejected (full refund+seats, no seller escrow), C buyer withdrawal, F duplicate approve (no double escrow), G duplicate reject (no double refund/seats), H same event_id idempotent, I price-mismatch rejection, passport dup same-package blocked (200/400), same passport different-package allowed (200/200), issue_visas while pending blocked.
- **BLOCKED next:** Rahal must confirm inbound contract for `booking.cancellation.approved/rejected` before Phase 2 (Super Admin cancellation-decision engine). Object storage (S3-compatible, env-configured, private) required before §4/§7 file uploads.

## P2P Cancellation decision engine — Phase 2 (backend) — DONE Aug 2026
Final contract (supersedes booking.cancellation.approved/rejected): Meraaj Super Admin is the FINAL financial authority; Rahal only submits a position.
- **Inbound `booking.cancellation.position`** (`integration.py::rahal_webhook`): match by `data.booking_ref`; stores `cancellation_position` + appends to `cancellation_positions[]` (position, executed_services, actual_costs_total, evidence…). NO money/seats movement. Audit `rahal_position`.
- **Super Admin queue**: `GET /api/admin/cancellations` (bookings with `cancellation_status="requested"`).
- **Decision**: `POST /api/admin/bookings/{id}/cancellation-decision` (require_admin), body `{decision: cancelled|kept, refund_amount, seller_compensation, reason}`. Validates settlement identity `refund + seller_compensation + platform_adjustment == original_amount (±0.01)`, `original_amount = amount_charged` (escrow anchor). Atomic claim on `cancellation_status=="requested"` → sets `decided`/`rejected` + `cancellation_final{}` (idempotent; duplicate → 400 pre-check or 409 race). On `cancelled`: reverses approval-time recognitions (seller escrow pending, platform fee/profit, marketer commission) then redistributes held amount (buyer refund + seller_compensation to available + platform_adjustment revenue) + releases seats + frees passports. On `kept`: no wallet change, booking stays active. Emits `meraaj.booking.cancellation_finalized` (v2 envelope) to Rahal.
- **E2E PASS**: approved→cancellation requested; position no_objection stored; position objection+executed costs stored; Full Refund (escrow removed, no seller comp, seats restored); Partial Refund (identity holds, seller paid comp); Keep Booking (no change, stays active); duplicate finalize blocked (400/409, no double refund); settlement mismatch → 400.
- **UNCHANGED**: HMAC/Secrets/_deliver/Outbox/Retry/serialization/meraaj.booking.created. Seller approval still does NOT release to seller available (settlement does).
- **NEXT (UI + features, not started):** حجوزاتي tabs, cancellation notifications, traveler documents + visa upload (needs S3 storage layer), account ledger, office verification.
