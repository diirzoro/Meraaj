...
## إضافة متطلب (2026-06): Mobile Native UX/UI
قرار المالك: البنية الحالية (React SPA + Capacitor 7 + Android shell) صحيحة ولا حاجة لـ Flutter/React Native.
المطلوب دائماً: **Same Backend + Same Business Logic + Dedicated Mobile UX/UI**.
- Desktop Dashboard يبقى للإدارة ولا يُحوَّل ليشبه الجوال؛ و`/m/*` تجربة تطبيق مستقلة للمستخدم.
- ممنوع تعديل Accounting Core / Batch1-2 / RBAC / Idempotency / `data-testid` من أجل التصميم.
- شاشة التذاكر تبقى "قريباً" موجّهة للعميل حتى استلام عقد API شركة النقل (لا تدفق وهمي).
- التقارير المحاسبية المحدودة بـ Account Scope يجب أن تُعلَّم صراحة كعرض جزئي.
- الحكم النهائي على تجربة الجوال بالعين والاستخدام على الهاتف، لا بنتائج الاختبارات وحدها.
